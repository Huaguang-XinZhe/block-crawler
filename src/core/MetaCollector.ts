...
import type { CollectionLink, SiteMeta } from "../types/meta";
...
export class MetaCollector {
	...
	async initialize(): Promise<void> {
		...
		if (existingMeta) {
			...
			const existingFreeBlocks = new Set(
				existingMeta.freeBlocks?.blockNames || [],
			);
	...
	addCollectionLinks(links: CollectionLink[]): void {
		...
		// 累加展示的总数
		this.meta.displayedTotalCount += links.reduce(
	...
	private mergeWithExisting(existingMeta: SiteMeta): SiteMeta {
		...
		// 合并 freePages（去重）
		const existingFreePages = new Set(existingMeta.freePages?.links || []);
		this.meta.freePages.links.forEach((link) => {
			existingFreePages.add(link);
		});
		...
		this.meta.freeBlocks.blockNames.forEach((name) => {
			existingFreeBlocks.add(name);
		});
	...
	private logSaveStats(finalMeta: SiteMeta, isComplete: boolean): void {
		...
		console.log(
			this.i18n.t("meta.collectedLinks", { count: finalMeta.totalLinks }),
		);
		console.log(
			this.i18n.t("meta.displayedTotal", {
				count: finalMeta.displayedTotalCount,
			}),
		);
		console.log(
			this.i18n.t("meta.actualTotal", { count: finalMeta.actualTotalCount }),
		);
		console.log(
			this.i18n.t("meta.freePages", { count: finalMeta.freePages.total }),
		);
		console.log(
			this.i18n.t("meta.freeBlocks", { count: finalMeta.freeBlocks.total }),
		);
		console.log(
			this.i18n.t("meta.duration", { duration: finalMeta.duration ?? 0 }),
		);
		const statusText =
			this.i18n.getLocale() === "zh"
				? isComplete
					? "是"
					: "否"
				: isComplete
	...
	static async loadFreePages(metaFile: string): Promise<string[]> {
		const meta = await MetaCollector.load(metaFile);
